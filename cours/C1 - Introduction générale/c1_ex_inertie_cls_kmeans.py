...
from clust_util import plotly_2d_clust, compute_inertia
...
# --------------------------
example_desc = "Classification avec algorithme (k-means)"
var_index = "country"
var_x = "exports"
var_y = "imports"
var_cls = "cls"
...
# Création d'une classification avec K-means
kmeans_model = KMeans(
    init="random",
...
data_classif_iw_fig = plotly_2d_clust(data_df=data_sel_df,
                                      data_weights=data_weights,
                                      title=example_desc +
                                      " (avec inertie intra)",
...
data_classif_ib_fig = plotly_2d_clust(data_df=data_sel_df,
                                      data_weights=data_weights,
                                      title=example_desc +
                                      " (avec inertie inter)",
                                      show_cls_center=True,
...
data_classif_inertia_fig = plotly_2d_clust(data_df=data_sel_df,
                                           data_weights=data_weights,
                                           title=example_desc +
                                           " (avec inertie intra et inter)",
                                           show_cls_center=True,
                                           show_within_inertia_line=True,
                                           show_between_inertia_line=True,
                                           var_x=var_x,
...
if show_plots:
    ...
    pio.show(data_classif_inertia_fig)
