...
1. Initialiser l'algorithme des moyennes mobiles sur les données =wine_ori_df= :
   #+BEGIN_SRC python :session :results silent :tangle td2.py :exports code
km_ori_1 = skc.KMeans(n_clusters=3)
   #+END_SRC
   - L'objet =km_ori_1= représente l'algorithme des moyennes mobiles. 
   ...
   - Notons que pour l'instant, aucun traitement n'a été réalisé puisqu'aucune données n'a été mise
     en relation avec notre objet =km_ori_1=.
...
km_ori_1.fit(wine_ori_df)
   ...
   - *Note :* On pourra également désigner l'objet =km_ori_1= comme le *modèle* =km_ori_1=. En effet, on
...
1. Après partitionnement, l'objet =km_ori_1= possède de nouveaux attributs. Déduire (si besoin avec
   l'aide de la documentation) la signification des attributs suivants :
   #+BEGIN_SRC python :session :results silent :tangle td2.py :exports code
km_ori_1.labels_
   #+END_SRC
   #+BEGIN_SRC python :session :results silent :tangle td2.py :exports code
km_ori_1.cluster_centers_
   #+END_SRC
   #+BEGIN_SRC python :session :results silent :tangle td2.py :exports code
km_ori_1.inertia_
   #+END_SRC

1. Calculer l'inertie expliquée par la partition obtenue avec le modèle =km_ori_1=.
...
km_ori_1_ie = 1 - km_ori_1.inertia_/wine_ori_it
   #+END_SRC

1. Représenter graphiquement la partition obtenue avec le modèle =km_ori_1= à l'aide d'un diagramme
   en paires et analyser le résultat.
   #+BEGIN_SRC python :session :results silent :tangle td2.py :exports code
px.scatter_matrix(wine_ori_df,
                  color=km_ori_1.labels_.astype(str), 
                  title="Partition km_ori_1").show()
   #+END_SRC

1. Analyser le profil des groupes obtenus avec la partition =km_ori_1=.
   #+BEGIN_SRC python :session :results silent :tangle td2.py :exports code
km_ori_1_prof = \
    pd.DataFrame(km_ori_1.cluster_centers_,
...
1. Représenter graphiquement les profils des groupes sous la forme d'un boxplot.
   #+BEGIN_SRC python :session :results silent :tangle td2.py :exports code
px.box(wine_ori_df, color=km_ori_1.labels_.astype(str),
       title="Profils des groupes obtenus par la classification km_ori_1").show()
   #+END_SRC

1. Étudier les paramètres de la classe =skc.kMeans=. Donner en particulier la signification des
   paramètres :
   - =init= ;
   - =n_init= ;
   - =max_iter= ;
...
1. Calculer l'inertie expliquée par la partition obtenue avec le modèle =km_ori_2=. Comparer ce
   résultat avec l'inertie expliquée par le modèle =km_ori_1=.
   #+BEGIN_SRC python :session :results silent :tangle td2.py :exports (if (eq td_corrige t) "code" "none")
# SOLUTION
# --------
...
2. Créer un modèle k-Means, nommé =km_cr_1=, permettant de partitionner les données en 3 groupes en
   utilisant le paramétrage par défaut de la classe =skc.kMeans=.
   #+BEGIN_SRC python :session :results silent :tangle td2.py :exports (if (eq td_corrige t) "code" "none")
# SOLUTION
# --------
km_cr_1 = skc.KMeans(n_clusters=3)
   #+END_SRC

3. Utiliser ensuite la méthode =fit= pour lancer le partitionnement sur les données =wine_cr_df= :
   #+BEGIN_SRC python :session :results silent :tangle td2.py :exports code
km_cr_1.fit(wine_cr_df)
   #+END_SRC

4. Calculer l'inertie expliquée par la parition obtenue avec le modèle =km_cr_1=.
   #+BEGIN_SRC python :session :results silent :tangle td2.py :exports (if (eq td_corrige t) "code" "none")
# SOLUTION
# --------
# Calcul de l'inertie totale des données
wine_cr_it = ((wine_cr_df - wine_cr_df.mean())**2).sum(axis=1).sum()
km_cr_1_ie = 1 - km_cr_1.inertia_/wine_cr_it
   #+END_SRC

5. Représenter graphiquement la partition obtenue avec le modèle =km_cr_1= à l'aide d'un diagramme
   en paires, analyser le résultat obtenu et comparer avec les partitions précédentes.
   #+BEGIN_SRC python :session :results silent :tangle td2.py :exports (if (eq td_corrige t) "code" "none")
# SOLUTION
# --------
px.scatter_matrix(wine_cr_df,
                  color=km_cr_1.labels_.astype(str), 
                  title="Partition km_cr_1").show()
   #+END_SRC

1. Mettre sous la forme d'un DataFrame le profil moyen des groupes obtenus avec la partition =km_cr_1=.
   #+BEGIN_SRC python :session :results silent :tangle td2.py :exports (if (eq td_corrige t) "code" "none")
# SOLUTION
# --------
km_cr_1_prof = \
    pd.DataFrame(km_cr_1.cluster_centers_,
...
px.box(wine_cr_df, color=km_cr_1.labels_.astype(str),
       title="Profils des groupes obtenus par la classification km_cr_1 (espace centré-réduit)").show()
# Espace original -> profils interprétables directement du point de vu métier
px.box(wine_ori_df, color=km_cr_1.labels_.astype(str),
       title="Profils des groupes obtenus par la classification km_cr_1 (espace original)").show()
   #+END_SRC
...
2. Réaliser un modèle k-Means, nommé =km_acp_1=, permettant de partitionner les données =wine_acp_2d_df=
   en 3 groupes. Visualiser la partition obtenue.
   #+BEGIN_SRC python :session :results silent :tangle td2.py :exports (if (eq td_corrige t) "code" "none")
# SOLUTION
# --------
km_acp_1 = skc.KMeans(n_clusters=3).fit(wine_acp_2d_df)
px.scatter(wine_acp_2d_df,
           x=0, y=1,
           color=km_acp_1.labels_.astype(str),
           labels={"0":"Axe ACP 1", "1":"Axe ACP 2"},
           title="Partition km_acp_1").show()
   #+END_SRC

2. En déduire le profil moyen des groupes obtenus dans l'espace centré-réduit.
   #+BEGIN_SRC python :session :results silent :tangle td2.py :exports (if (eq td_corrige t) "code" "none")
# SOLUTION
# --------
km_acp_1_prof = wine_cr_df.groupby(km_acp_1.labels_).mean()
   #+END_SRC
...
px.box(wine_acp_2d_df, color=km_acp_1.labels_.astype(str),
       title="Profils des groupes obtenus par la classification km_acp_1 (espace ACP)").show()
# Espace centré-réduit -> profils interprétables mais ne pas oublier de tenir compte du centrage/réduction
px.box(wine_cr_df, color=km_acp_1.labels_.astype(str),
       title="Profils des groupes obtenus par la classification km_acp_1 (espace centré-réduit)").show()
# Espace original -> profils interprétables directement du point de vu métier
px.box(wine_ori_df, color=km_acp_1.labels_.astype(str),
       title="Profils des groupes obtenus par la classification km_acp_1 (espace original)").show()

   #+END_SRC

2. Calculer l'inertie expliquée par la partition =km_acp_1=.
   #+BEGIN_SRC python :session :results silent :tangle td2.py :exports (if (eq td_corrige t) "code" "none")
# SOLUTION
# --------
...
# Utilisation de la fonction pour calculer l'inertie expliquée de la partition km_acp_1
km_acp_1_iw, km_acp_1_ie = eval_partition(wine_cr_df, km_acp_1.labels_)
   #+END_SRC
