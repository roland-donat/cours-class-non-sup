...
#+ATTR_HTML: :width 50%
#+CAPTION: /A woman datascientist performing machine learning to bet on soccer teams, manga style/ (source: générée par [[https://openai.com/dall-e-2/][DALL-E]])
...
import sklearn.mixture as skm         # Package sklearn dédié aux modèles de mélange
...
cah_cr_def.n_leaves_
   #+END_SRC
   #+BEGIN_SRC python :session :results silent :tangle td3.py :exports code
cah_cr_def.n_connected_components_
   #+END_SRC
   #+BEGIN_SRC python :session :results silent :tangle td3.py :exports code
cah_cr_def.children_
   #+END_SRC
   
1. Représenter graphiquement la partition obtenue avec le modèle =cah_cr_def= à l'aide d'un diagramme
   en paires et analyser le résultat.
   #+BEGIN_SRC python :session :results silent :tangle td3.py :exports code
px.scatter_matrix(wine_cr_df,
                  color=cah_cr_def.labels_.astype(str), 
                  title="Partition cah_cr_def")
   #+END_SRC
...
4. Étudier l'aide de la classe [[https://scikit-learn.org/stable/modules/generated/sklearn.cluster.AgglomerativeClustering.html#sklearn.cluster.AgglomerativeClustering][=AgglomerativeClustering=]] afin de comprendre le rôle des
   principaux paramètres de la classe, à savoir =n_clusters=, =metric= et =lingkage=.

5. Construire une nouvelle hiérarchie =cah_cr_ward= sur les données =wine_cr= afin de produire une partition en 3
...
px.box(wine_cr_df, color=cah_cr_ward.labels_.astype(str),
       title="Profils de la partition cah_cr_ward")
   #+END_SRC
...
cah_cr_ward_dendro_fig.update_layout(
    title_text=f"CAH avec distance d'aggrégation Ward et distance entre individus euclidienne",
...
3. Représenter graphiquement la partition obtenue avec le modèle =cah_cr_1= à l'aide d'un diagramme
   en paires et analyser le résultat.
   #+BEGIN_SRC python :session :results silent :tangle td3.py :exports (if (eq td_corrige t) "code" "none")
# SOLUTION
# --------
px.scatter_matrix(wine_cr_df,
                  ...
                  title="Partition cah_cr_single")
   #+END_SRC

1. Représenter graphiquement les profils des groupes sous la forme d'un boxplot.
   #+BEGIN_SRC python :session :results silent :tangle td2.py :exports code
px.box(wine_cr_df, color=cah_cr_single.labels_.astype(str),
       title="Profils de la partition cah_cr_single")
   #+END_SRC
...
cah_cr_single_dendro_fig.update_layout(
    title_text=f"CAH avec distance d'aggrégation du lien minimum",
    width=1600, height=900)
   #+END_SRC
...
px.box(wine_cr_df, color=cah_cr_complete.labels_.astype(str),
       title="Profils de la partition cah_cr_complete")
...
cah_cr_complete_dendro_fig.update_layout(
    title_text=f"CAH avec distance d'aggrégation du lien maximum",
    width=1600, height=900)
   #+END_SRC
...
cah_cr_average_dendro_fig.update_layout(
    title_text=f"CAH avec distance d'aggrégation du lien moyen",
    width=1600, height=900)
   #+END_SRC
...
px.scatter_matrix(soccer_mean_df,
                  dimensions=['FTHG', 'HTHG', 'HS', 'HST', 'HC', 'HF', 'HY', 'HR'],
                  hover_name=soccer_mean_df.index,
                  color=cah_soccer_ward.labels_.astype(str), 
                  title="Partition en 4 groupes des équipes de football obtenue par CAH.")
...
cah_soccer_ward_dendro_fig.update_layout(
    title_text=f"Dendrogramme de la CAH sur les équipes de football avec la distance de Ward",
    width=1600, height=900)
   #+END_SRC
